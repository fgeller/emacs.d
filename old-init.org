...
** Shell
   ...
   #+begin_src emacs-lisp
     (setenv "SHELL" (if mac-p "/usr/local/bin/zsh" "/usr/bin/zsh"))
   #+end_src
...
** Sources path
   ...
   #+begin_src emacs-lisp
     (setq find-function-C-source-directory
           (expand-file-name "~/site-lisp/emacs-sources/src"))
   #+end_src
...
** Load path
   ...
   #+begin_src emacs-lisp
     (add-to-list 'load-path (expand-file-name "~/site-lisp/src/helm"))
     (add-to-list 'load-path (expand-file-name "~/site-lisp/src/emacs-async"))
   #+end_src
...
** Smartparens
   ...
   #+begin_src emacs-lisp
     ...
     (sp-local-pair '(clojure-mode emacs-lisp-mode) "`(" ")")
   ...
   #+begin_src emacs-lisp
     (setq  pretty-symbol-patterns
            '((?λ lambda "\\<lambda\\>" (emacs-lisp-mode inferior-lisp-mode lisp-mode scheme-mode python-mode inferior-python-mode))
              ...
              (?∀ lambda "\\<forall\\>" (scala-mode scala2-mode))
              (?⇾ lambda "->" (php-mode))))
   #+end_src
...
** Scala
   ...
   #+begin_src emacs-lisp
     (defun er/add-scala-mode-expansions ()
       (make-variable-buffer-local 'er/try-expand-list)
       (setq er/try-expand-list (append
                                 er/try-expand-list
                                 '(er/mark-scala-type-parameter-or-application
                                   er/mark-scala-scope-signature))))

     (add-hook 'scala-mode-hook 'er/add-scala-mode-expansions)

     (defun er/mark-scala-scope-signature ()
       (interactive)
       ;; (re-search-backward "class\\|object\\|trait\\|case")
       )


     (defun er/mark-scala-type-parameter-or-application ()
       (interactive)
       (let ((start-point (point)))
...
** SQL
   ...
   Default to highlight MySQL keywords.
...
** Editing text
   ...
   #+begin_src emacs-lisp
     ...
     (define-key global-map (kbd "C-d C-l") 'c/kill-whole-line)
   #+end_src
...
** Email
   ...
   #+begin_src emacs-lisp
     (eval-after-load 'notmuch
       '(progn
          (define-key notmuch-search-mode-map (kbd "C-d") 'c/notmuch-search-archive)
          (define-key notmuch-show-mode-map (kbd "C-d") 'c/notmuch-show-archive)))
   #+end_src
...
*** todochiku
    ...
    #+begin_src emacs-lisp
      (setq todochiku-icons-directory (expand-file-name "~/site-lisp/src/todochiku-icons/"))
    #+end_src
...
*** Notmuch
    ...
    Helper to archive from notmuch's search view.

    #+begin_src emacs-lisp
      (defun c/notmuch-search-archive ()
        (interactive)
        (notmuch-search-tag '("-unread" "-inbox"))
        (notmuch-search-refresh-view))
    #+end_src

    Helper to archive from notmuch's show view.

    #+begin_src emacs-lisp
      (defun c/notmuch-show-archive ()
        (interactive)
        (notmuch-show-tag '("-unread" "-inbox"))
        (notmuch-kill-this-buffer)
        (notmuch-search-refresh-view))
    #+end_src

    Setup helper to find addresses.
    ...
    Tweak address completion to work with helm.
...
* Misc helpers
  ...
  #+begin_src emacs-lisp
    (defun c/start-scalari ()
      (interactive)
      (cd "~/code/scala/scalariform-daemon")
      (compile "sbt run"))
  #+end_src
...
* Prodigy
  ...
  #+begin_src emacs-lisp
    (prodigy-define-service
      ...
      :command "sbt"
      :args '("run")
      :cwd "~/code/scala/scalariform-daemon")
  #+end_src
