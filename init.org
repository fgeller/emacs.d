...
** grep
   ...
   #+begin_src emacs-lisp
     ...
     (add-to-list 'grep-find-ignored-directories ".ensime_lucene")
   #+end_src
...
** C-c Map
   ...
   #+begin_src emacs-lisp
     ...
     (define-key global-map (kbd "C-c md") 'c/popwin-dired)
...
*** popwin
   ...
   Helper to start dired via popwin.

   #+begin_src emacs-lisp
     (defun c/popwin-dired ()
       (interactive)
       (popwin:display-buffer-1
        (save-window-excursion
          (dired-noselect (or default-directory (expand-file-name "~/"))  nil))
        :default-config-keywords '(:position :left :width 40)))
   #+end_src
