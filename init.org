     ...
     #+begin_src emacs-lisp
       ...
       (defun gm-maybe-release-override-map ()
         (when gm-stored-overriding-local-map
           (when gm-debug (message "[gm] releasing override-local-map to %s" (car gm-stored-overriding-local-map)))
       ...
       (defun gm-maybe-override-map ()
         (when gm-debug (message "[gm] maybe override map, in buffer %s with god-global-mode %s major-mode %s" (buffer-name) god-global-mode major-mode))
         (when (and
                god-global-mode
                (not (eq overriding-local-map god-local-mode-map))
                (-any? 'derived-mode-p gm-special-override-modes))
           (when gm-debug (message "[gm] activating override map, storing %s" `(,overriding-local-map . t)))
