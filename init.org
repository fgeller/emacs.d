...
** GNU Global
   ...
   Monkey-patch helm-gtags to color-code the candidates.

   #+begin_src emacs-lisp
     (eval-after-load 'helm-gtags
     '(progn
       (setq helm-source-gtags-tags
       `((name . "GNU GLOBAL")
     ...
     (defun helm-gtags-tags-candidate-transformer (candidate)
       (when (string-match "\\(.+?\\):\\(.+?\\):\\(.+\\)" candidate)
         (format "%s:%s:%s"
                 (propertize (match-string 1 candidate) 'face 'font-lock-keyword-face)
                 (propertize (match-string 2 candidate) 'face 'font-lock-variable-name-face)
                 (propertize (match-string 3 candidate) 'face 'font-lock-comment-face))))))
   #+end_src
