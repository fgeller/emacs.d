...
**** god-mode
     ...
     #+begin_src emacs-lisp
       ...
       (defvar gm-debug t)
       (setq gm-special-override-modes '(magit-mode dired-mode))

       (add-hook 'god-mode-disabled-hook 'gm-maybe-release-override-map)
       (add-hook 'god-mode-enabled-hook 'gm-maybe-override-map)

       (defun gm-maybe-release-override-map ()
         (when gm-stored-overriding-local-map
           (when gm-debug (message "releasing override-local-map to %s" (car gm-stored-overriding-local-map)))
       ...
       (defun gm-maybe-override-map ()
         (when gm-debug (message "maybe override map, in buffer %s with god-global-mode %s major-mode %s" (buffer-name) god-global-mode major-mode))
         (when (and
                god-global-mode
                (not (eq overriding-local-map god-local-mode-map))
                (-any? 'derived-mode-p gm-special-override-modes))
           (when gm-debug (message "activating override map, storing %s" `(,overriding-local-map . t)))
...
** Scala
   ...
   #+begin_src emacs-lisp
     (defun helm-gtags-action-import-tag (elm)
       (let* ((filename (first (split-string elm ":")))
     ...
     (setcdr (assq 'action helm-source-gtags-tags)
             '(("Jump to tag" . helm-gtags-action-openfile)
               ("Import tag" . helm-gtags-action-import-tag)))
   #+end_src
   ...
   #+begin_src emacs-lisp
     (defun c/god-mode-update-cursor ()
       (cond (god-local-mode (progn
                               ...
                               (set-face-background 'fringe (if terminal-p "white" "#fcf4dc"))))
             (t (set-face-background 'fringe (if terminal-p "black" "#0a2832")))))
...
** Text editing
   ...
   #+begin_src emacs-lisp
     (define-key global-map (kbd "C-z C-z") 'c/zap-up-to-char-backwards)
     (define-key global-map (kbd "C-z C-x") 'zap-up-to-char)
     (define-key global-map (kbd "C-z C-h") 'sp-kill-symbol)
     (define-key global-map (kbd "C-z C-w") 'c/subword-kill)
   #+end_src
