...
*** require'd
   ...
   #+end_src
    ...
    #+begin_src emacs-lisp
      (sp-local-pair '(emacs-lisp-mode magit-log-edit-mode message-mode) "'" nil :actions nil)
       ...
       (when buffer-file-name
         (let* ((err-regex "line [0-9]+, col [0-9]+, found") ;; stolen from flymake-json
                ...
                (new-contents (with-temp-buffer
                                ...
                                (goto-char (point-min))
                                (if (looking-at-p err-regex) (progn (message "should be true") nil)
                                  (buffer-substring (point-min) (point-max))))))
           (when new-contents
             (let ((old-point (point)))
   ...
   #+begin_src emacs-lisp
     (defun c/kill-whole-line ()
       ...
       (let ((kill-whole-line t))
         (kill-line)))
     (define-key global-map (kbd "C-c k") 'c/kill-whole-line)
   #+end_src
